...
Ventas::Ventas(FarmaSystem::SistemaFarmacia* sistemaCentral, QWidget *parent)
    : QDialog(parent)
    , ui(new Ui::Ventas)
{
    ui->setupUi(this);
...
void Ventas::on_btnVerificar_clicked()
{
    ...
    FarmaSystem::Cliente* cliente = sistema->getListaClientes().buscarPorCedula(idCliStr);
    if (cliente == nullptr) {
        QMessageBox msgBox(this);
    ...
    FarmaSystem::Medicamento* med = sistema->getListaMedicamentos().buscarPorId(std::stoi(idMedStr));
    if (med == nullptr) {
        QMessageBox msgBox(this);
        msgBox.setIcon(QMessageBox::Critical);
        msgBox.setWindowTitle("Error");
...
void Ventas::on_btnFacturar_clicked()
{
    ...
    sistema->getListaVentas().agregar(nuevaVenta);
    sistema->getListaMedicamentos().actualizarStockPorId(med->getId(), med->getStock() - cantidad);
...
void Ventas::actualizarTablaHistorial(int filtroClienteId)
{
    ...
    FarmaSystem::NodoVenta* aux = sistema->getListaVentas().getPrimero();
    ...
    } while (aux != sistema->getListaVentas().getPrimero());


    double acumulado = (filtroClienteId == -1)
                           ? sistema->getListaVentas().calcularAcumuladoTotal()
                           : sistema->getListaVentas().calcularAcumuladoPorCliente(filtroClienteId);

    ui->lblMontoAcu->setText("₡" + QString::number(acumulado, 'f', 2));

    this->nodoExplorador = sistema->getListaVentas().getPrimero();
...
void Ventas::mostrarVentaEnExplorador() {
    if (nodoExplorador == nullptr || nodoExplorador->getVenta() == nullptr) {
        ui->lbnlRegistroActual->setText("[Ninguna Venta]");
        return;
    }
